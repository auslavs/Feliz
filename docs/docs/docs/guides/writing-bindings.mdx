...
We can see the library assumes an import on which we can call the `readCSV` function. After a quick search we can find that `const dfd = require("danfojs-node")` is similiar to `import * as dfd from "danfojs"` in ES6, which is what we want to use in the browser.
